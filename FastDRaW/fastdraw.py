...
class Segmenter():
    ...
    def __init__(
        ...
        assert (len(downsampled_size) ==
                2), 'incorrect value of downsampled_size, it should contain 2 elements: e.g., [100,100].'
        assert (
            downsampled_size[0] > 0), 'incorrect value, downsampled_size must be > 0.'
        assert (
            downsampled_size[1] > 0), 'incorrect value, downsampled_size must be > 0.'
    ...
    def _buildGraph(self, image, RW_BETA):
        ...
        weights = np.exp(- RW_BETA *
                         np.r_[gr_deep, gr_right, gr_down]**2) + 1e-6
    ...
    def _coarse_random_walker(self, ds_labels, ds_maskROI, target_label):
        ...
        """
        unlabeled = np.ravel_multi_index(
            np.where(
                (ds_labels == 0) & (ds_maskROI)),
            ds_labels.shape)
    ...
    def update(self, labels, target_label=1, k=0.5):
        ...
        ds_probability = self._coarse_random_walker(
            ds_labels, ds_maskROI, target_label)
        ...
        for i in range(self.ds_size[2]):
            ds_contour[..., i] = (binary_dilation(
                ds_mask[..., i], struct_el2D) - ds_mask[..., i]).astype(np.bool)
            ds_maskROI[..., i] = binary_dilation(
                ds_contour[..., i], struct_el2D, iterations=2)
        ...
        probability = self._refinement_random_walker(
            ds_labels, ds_maskROI, ds_mask, target_label)
